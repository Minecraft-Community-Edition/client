...
void PlayerList::toggleDimension(shared_ptr<ServerPlayer> player, int targetDimension)
{
	...
	{
		// Entering The Aether â€” reuse a generic progress message
	...
	else if (player->dimension == 0)
	{
		if (lastDimension == -1)
		{
			// Coming from Nether â€” scale up
			xt *= scale;
			zt *= scale;
		}
		// Coming from Aether or End â€” keep coordinates as-is (1:1)
        player->moveTo(xt, player->y, zt, player->yRot, player->xRot);
        if (player->isAlive())
		{
            oldLevel->tick(player, false);
	...
	{
		// Entering The Aether â€” 1:1 coordinate mapping with Overworld
		player->moveTo(xt, player->y, zt, player->yRot, player->xRot);
		if (player->isAlive())
		{
			oldLevel->tick(player, false);
